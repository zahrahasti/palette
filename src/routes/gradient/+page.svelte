	...
	>
		<section class="z-20  relative range-slider">
			...
			<div class="absolute top-0 w-full">
				...
				/>
				<div
					style="background-image:{type}-gradient({type === 'radial'
						? `circle`
						: `${gradient.Rotation.value}deg`}, {color_1} {color_num_1}%,{color_2} {color_num_2}%);"
		...
		<div class="grid mt-20 grid-cols-2 gap-8">
			<Box details={gradient.Color}>
				...
				<input
					type="color"
					on:input={(e) => {
						if (active === 1) thump_color_1 = color_1 = e.target?.value;
						else thump_color_2 = color_2 = e.target?.value;
					}}
