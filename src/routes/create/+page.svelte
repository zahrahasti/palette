<script lang="ts">
	...
	function userSelectedInput(e: Event) {
		...
		console.log(id);
		const inputColors = document.querySelectorAll(
			'input[type=color]'
	...
	<div class="flex gap-4 flex-wrap relative justify-center">
		...
		>
			<div
				...
				class="relative cursor-pointer bg-[#BBBBBB]"
				data-index="0"
				data-color="#BBBBBB"
			></div>
			<div
				on:click={userSelectedInput}
				class="relative cursor-pointer bg-[#CCCCCC]"
				data-index="1"
				data-color="#CCCCCC"
			></div>
			<div
				on:click={userSelectedInput}
				class="relative cursor-pointer bg-[#DDDDDD]"
				data-index="2"
				data-color="#DDDDDD"
			></div>
			<div
				on:click={userSelectedInput}
				class="relative cursor-pointer bg-[#EEEEEE]"
				data-index="3"
				data-color="#EEEEEE"
			></div>
