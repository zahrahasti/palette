...
export function createPalette(colors,parent,type){
    ...
    colors.map(color=>{
      ...
      child1=`
      <div style="background:#${color.color_1}" class="pallete--color  grid grid-rows-16 w-full  text-[1.6rem] md:text-[1.4rem] xl:text-[1.5rem] text-white  bg-[#${color.color_1}] aspect-square bg-red-500 top-0 rounded-[1rem] overflow-hidden  pallete--color__1" data-id="${color.id}">
      ...
      if(type==="clicked"){
        div.classList.remove("container--pallete__color","scale-[.85]","sm:scale-100")
